                    ...
                    <div class="space-y-1">
                        ...
                        <div class="flex flex-col gap-0.75">
                            ...
                            <div class="flex items-center gap-2 text-sm font-medium text-(--muted-foreground)">
                                ...
                                <template x-if="title.runtime">
                                    <span x-text="formatRuntime(title.runtime)"
                                        class="tracking-wider before:content-['\00B7'] before:mr-2"></span>
                                </template>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 lg:gap-8">
                    <div>
                        ...
                        <div class="divide-y divide-(--border) border-y border-(--border)">
                            ...
                            <!-- Director -->
                            <template x-if="title.credits?.crew?.filter(c => c.job === 'Director').length > 0">
                                <div class="flex items-baseline gap-3 sm:gap-4 py-3">
                                    <dt class="shrink-0 font-bold text-sm sm:text-base w-14 sm:w-16">Director</dt>
                                    <dd class="flex flex-wrap gap-x-1.5 text-sm sm:text-base">
                                        <template
                                            x-for="(person, i) in (title.credits?.crew ?? []).filter(c => c.job === 'Director')"
                                            ...
                                            <span>
                                                ...
                                                <template
                                                    x-if="i !== (title.credits?.crew ?? []).filter(c => c.job === 'Director').length - 1">
                                                    <span
                            ...
                            <!-- Writers -->
                            <template x-if="title.credits?.crew?.filter(c => c.job === 'Screenplay').length > 0">
                                <div class="flex items-baseline gap-3 sm:gap-4 py-3">
                                    <dt class="shrink-0 font-bold text-sm sm:text-base w-14 sm:w-16">Writers</dt>
                                    <dd class="flex flex-wrap gap-x-1.5 text-sm sm:text-base">
                                        <template
                                            x-for="(person, i) in (title.credits?.crew ?? []).filter(c => c.job === 'Screenplay').slice(0, 3)"
                                            :key="person.credit_id">
                                            <span>
                                                <a href="#"
                                                    class="text-blue-500 dark:text-blue-400 hover:underline font-medium"
                                                    x-text="person.name"></a>
                                                <template
                                                    x-if="i !== (title.credits?.crew ?? []).filter(c => c.job === 'Screenplay').slice(0, 3).length - 1">
                                                    <span
                                                        class="font-bold text-(--muted-foreground)">&nbsp;&middot;&nbsp;</span>
                                                </template>
                                            </span>
                            ...
                            <div class="flex items-baseline gap-3 sm:gap-4 py-3">
                                ...
                                <dd class="flex flex-wrap items-center gap-x-1.5 text-sm sm:text-base">
                    ...
                    <div class="lg:w-64 lg:row-span-2 flex flex-col gap-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
                            <a :href="`https://streamex.sh/watch/movie/${title.id}`" target="_blank"
                                class="contents">
                            ...
                            <a href="#" :data-title="title.title || title.name" x-data="telegram"
                            ...
                            <a :href="`https://www.themoviedb.org/movie/${title.id}`" target="_blank"
                                class="contents">
                                <x-ui.button variant="secondary" size="lg"
                                    class="rounded-full! cursor-pointer!">
                        ...
                        <dl class="divide-y divide-(--border) border-y border-(--border)">
                            ...
                            <template x-if="title.credits?.crew?.filter(c => c.job === 'Director').length > 0">
                                <div class="flex items-baseline gap-3 sm:gap-4 py-3">
                                    ...
                                    <dd class="flex flex-wrap gap-x-1.5 text-sm sm:text-base">
                                        <template
                                            x-for="person in (title.credits?.crew ?? []).filter(c => c.job === 'Director')"
                                            :key="person.credit_id">
                                            <a href="#"
                                                class="text-blue-500 dark:text-blue-400 hover:underline font-medium"
                                                x-text="person.name"></a>
                                        ...
                                        <template
                                            ...
                                            <span class="flex items-center gap-0.5">
                                                <a href="#"
                                                    class="text-blue-500 dark:text-blue-400 hover:underline font-medium"
                                                    x-text="person.name"></a>
                                                <span x-show="i < 2" class="text-(--muted-foreground)">,</span>
                                            </span>
                                        </template>
                                    </dd>
                                </div>
